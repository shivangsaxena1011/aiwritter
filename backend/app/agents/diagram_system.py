...
class DiagramPlanner:
    ...
    async def plan_diagram(
        ...
        """Evaluates whether this section benefits from visual diagrams."""
...
class DiagramGenerator:
    ...
    async def generate_asset(
        ...
        # 2. AI Illustration (Imagen 3)
        ai_prompt = plan.get("ai_prompt") or f"Professional university textbook schematic of {topic_title}"
        img_path = os.path.join(output_dir, f"fig_{file_id}.png")
        img_result = await self.ai.generate_image(ai_prompt, img_path)

        if img_result.get("success") and img_result.get("path"):
            return {
                "success": True,
        ...
        # 3. Fallback: Formatted Diagram Placeholder Box
        placeholder_text = self.create_placeholder_box(caption, ai_prompt, plan.get("description", ""))
        return {
            "success": False,
            "path": None,
            "caption": caption,
    ...
    def _generate_scientific_plot(self, plan: Dict[str, Any], output_path: str, title: str) -> bool:
        ...
        try:
            ...
            ax.plot(x, y_ideal, label="Reversible Nernst Potential ($E_{rev}$)", color="#2563eb", lw=2, linestyle="--")
            ...
            ax.set_xlabel("Normalized Load Parameter ($J / J_{max}$)", fontsize=10, labelpad=8)
            ax.set_ylabel("Specific Potential ($V_{cell}$ [V])", fontsize=10, labelpad=8)
